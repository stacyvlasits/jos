...
$id_query="select quizzes.quiz_name from quizzes where id=$q_id"; 
$q=$DB->Query($id_query); 
?>
...
<? 
  ...
  echo "<h2>Quiz Title: $quiz_name";
   ...
   $class_query = "select class from professor where id = $p_id";
...
<? 
if($section==no){
	$sql="select * from quizresults where q_id=$q_id and class LIKE '$class_name' order by l_name asc";
}else{
	$sql="select * from quizresults where q_id=$q_id and class LIKE '$class_name' and section='$section' order by l_name asc";
}
